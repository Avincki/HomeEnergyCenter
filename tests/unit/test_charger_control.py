...
# ----- start-failure backoff + dwell -------------------------------------------

# Strong export that always clears the resume threshold, so these tests isolate
# the backoff state machine from the signal calculation.
_STRONG = {"grid_power_w": -6000.0, "battery_power_w": 0.0}
_T0 = datetime(2026, 5, 22, 12, 0, tzinfo=UTC)


def test_failed_start_backs_off_after_timeout() -> None:
    ctrl = ChargerController(_config(start_timeout_s=120.0, failed_start_cooldown_s=300.0))
    # Resume: signal covers, car commanded at 6 A.
    cmd = ctrl.decide(_inputs(timestamp=_T0, actual_current_a=0.0, **_STRONG))
    assert not cmd.paused and cmd.target_a == 6.0
    # Still 0 A drawn 60 s later -> within grace, holds the command (no backoff).
    cmd = ctrl.decide(
        _inputs(timestamp=_T0 + timedelta(seconds=60), actual_current_a=0.0, **_STRONG)
    )
    assert not cmd.paused and cmd.target_a == 6.0
    # Past start_timeout (130 s) and still not drawing -> failed start, pause.
    cmd = ctrl.decide(
        _inputs(timestamp=_T0 + timedelta(seconds=130), actual_current_a=0.0, **_STRONG)
    )
    assert cmd.paused and cmd.target_a == 0.0
    assert "not drawing" in cmd.reason


def test_cooldown_blocks_resume_after_failed_start() -> None:
    ctrl = ChargerController(_config(start_timeout_s=120.0, failed_start_cooldown_s=300.0))
    ctrl.decide(_inputs(timestamp=_T0, actual_current_a=0.0, **_STRONG))  # resume 6 A
    fail = ctrl.decide(
        _inputs(timestamp=_T0 + timedelta(seconds=130), actual_current_a=0.0, **_STRONG)
    )
    assert fail.paused
    # 100 s into the 300 s cooldown: signal still covers, but resume is held off.
    cmd = ctrl.decide(
        _inputs(timestamp=_T0 + timedelta(seconds=230), actual_current_a=0.0, **_STRONG)
    )
    assert cmd.paused and "cooldown" in cmd.reason


def test_resumes_after_cooldown_expires() -> None:
    ctrl = ChargerController(_config(start_timeout_s=120.0, failed_start_cooldown_s=300.0))
    ctrl.decide(_inputs(timestamp=_T0, actual_current_a=0.0, **_STRONG))  # resume
    ctrl.decide(
        _inputs(timestamp=_T0 + timedelta(seconds=130), actual_current_a=0.0, **_STRONG)
    )  # fail -> cooldown until +430 s
    cmd = ctrl.decide(
        _inputs(timestamp=_T0 + timedelta(seconds=500), actual_current_a=0.0, **_STRONG)
    )
    assert not cmd.paused and cmd.target_a == 6.0


def test_draw_resets_failed_start_tracking() -> None:
    ctrl = ChargerController(_config(start_timeout_s=120.0))
    ctrl.decide(_inputs(timestamp=_T0, actual_current_a=0.0, **_STRONG))  # resume 6 A
    # Long after the resume, but the car IS drawing -> no failed start; the
    # not-drawing timer slides to now and the failure count stays 0.
    cmd = ctrl.decide(
        _inputs(timestamp=_T0 + timedelta(seconds=300), actual_current_a=6.0, **_STRONG)
    )
    assert not cmd.paused
    assert ctrl._consecutive_failed_starts == 0
    assert ctrl._offer_since == _T0 + timedelta(seconds=300)


def test_detach_resets_backoff() -> None:
    ctrl = ChargerController(_config(start_timeout_s=120.0, failed_start_cooldown_s=300.0))
    ctrl.decide(_inputs(timestamp=_T0, actual_current_a=0.0, **_STRONG))  # resume
    ctrl.decide(
        _inputs(timestamp=_T0 + timedelta(seconds=130), actual_current_a=0.0, **_STRONG)
    )  # fail -> cooldown
    # Car unplugged -> backoff cleared (fresh plug-in next time).
    ctrl.decide(_inputs(timestamp=_T0 + timedelta(seconds=140), car_attached=False))
    assert ctrl._cooldown_until is None
    assert ctrl._consecutive_failed_starts == 0
    # Re-plug: resumes immediately, no lingering cooldown.
    cmd = ctrl.decide(
        _inputs(timestamp=_T0 + timedelta(seconds=150), actual_current_a=0.0, **_STRONG)
    )
    assert not cmd.paused and cmd.target_a == 6.0


def test_consecutive_failed_starts_escalate_to_backoff() -> None:
    ctrl = ChargerController(
        _config(
            start_timeout_s=120.0,
            failed_start_cooldown_s=300.0,
            max_consecutive_failed_starts=2,
            backoff_cooldown_s=1800.0,
        )
    )

    def attempt_and_fail(start: datetime) -> object:
        ctrl.decide(_inputs(timestamp=start, actual_current_a=0.0, **_STRONG))  # resume
        return ctrl.decide(
            _inputs(timestamp=start + timedelta(seconds=130), actual_current_a=0.0, **_STRONG)
        )

    f1 = attempt_and_fail(_T0)
    assert f1.paused and "300" in f1.reason  # type: ignore[attr-defined]
    # Second failure hits the max (2) -> escalates to the 1800 s backoff. Start the
    # next attempt after the first 300 s cooldown has lapsed.
    f2 = attempt_and_fail(_T0 + timedelta(seconds=500))
    assert f2.paused and "1800" in f2.reason and "backing off" in f2.reason  # type: ignore[attr-defined]


def test_down_tick_pause_sets_resume_cooldown() -> None:
    ctrl = ChargerController(_config(resume_cooldown_s=90.0))
    ctrl._target_a = 6.0
    # Drawing 6 A but importing -> down-tick to 5 -> below min -> pause w/ cooldown.
    cmd = ctrl.decide(
        _inputs(timestamp=_T0, grid_power_w=600.0, battery_power_w=0.0, actual_current_a=6.0)
    )
    assert cmd.paused and cmd.target_a == 0.0
    # 30 s later a covering signal would resume, but the 90 s cooldown holds it.
    cmd = ctrl.decide(_inputs(timestamp=_T0 + timedelta(seconds=30), **_STRONG))
    assert cmd.paused and "cooldown" in cmd.reason
    # After the cooldown it resumes.
    cmd = ctrl.decide(_inputs(timestamp=_T0 + timedelta(seconds=120), **_STRONG))
    assert not cmd.paused and cmd.target_a == 6.0


def test_manual_target_clears_active_cooldown() -> None:
    # A manual Send during a backoff cooldown overrides it (explicit user action).
    ctrl = ChargerController(_config(start_timeout_s=120.0, failed_start_cooldown_s=300.0))
    ctrl.decide(_inputs(timestamp=_T0, actual_current_a=0.0, **_STRONG))  # resume
    ctrl.decide(
        _inputs(timestamp=_T0 + timedelta(seconds=130), actual_current_a=0.0, **_STRONG)
    )  # fail -> cooldown
    assert ctrl._cooldown_until is not None
    ctrl.adopt_manual_target(10.0)
    assert ctrl._cooldown_until is None
    assert ctrl._consecutive_failed_starts == 0
