...
| `ssh alex@homecenter.local` → "Could not resolve hostname" (used to work) | `.local` is mDNS, which is flaky on Windows — a network set to **Public** blocks it, the Bonjour/mDNS responder may have stopped, or the PC and Pi ended up on different subnets/VLANs (mDNS is link-local and won't cross them). Since the Pi is on Tailscale, **SSH to the tailnet name instead** — it doesn't need mDNS and works from any network: `ssh alex@homecenter.tail7b791a.ts.net` (the same host the dashboard uses; this is the canonical SSH target). Confirm both ends are up with `tailscale status`; you can also SSH the Pi's `100.x` Tailscale IP or its LAN `192.168.129.x`. To get `.local` back: set the PC's network profile to **Private**, ensure the PC and Pi share a subnet, and check `sudo systemctl status avahi-daemon` on the Pi. |
